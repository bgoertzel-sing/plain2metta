...
class ValidationRecordTests(unittest.TestCase):
    ...
    def test_validation_obligation_record_validation_refuses_malformed_entries(self):
        ...
        doc = SpecDocument(validation_obligations=[None, ["not-an-obligation"], valid])
        from specatom_hs.validators import validate_document
        ...
        records = [
            (record.status, record.evidence)
            for record in doc.checks
            if record.property == "validation-obligation-has-valid-record-type"
        ]
        self.assertEqual(
            records,
            [
                (CheckStatus.FAIL, "unsupported validation obligation record type=NoneType"),
                (CheckStatus.FAIL, "unsupported validation obligation record type=list"),
                (CheckStatus.PASS, "record type=ValidationObligation"),
            ],
        )
    ...
    def test_plain_item_record_validation_refuses_malformed_entries(self):
        ...
        doc = SpecDocument(items=[None, ["not-an-item"], valid])
        from specatom_hs.validators import validate_document
        ...
        records = [
            (record.status, record.evidence)
            for record in doc.checks
            if record.property == "item-has-valid-record-type"
        ]
        self.assertEqual(
            records,
            [
                (CheckStatus.FAIL, "unsupported item record type=NoneType"),
                (CheckStatus.FAIL, "unsupported item record type=list"),
                (CheckStatus.PASS, "record type=PlainItem"),
            ],
        )

    def test_section_record_validation_refuses_malformed_entries(self):
        span = SourceSpan("span-valid", "file-valid", 0, 1, 1, 1)
        valid = Section("section-valid", "file-valid", "Definitions", "definitions", 0, span)
        doc = SpecDocument(sections=[None, ["not-a-section"], valid])
        from specatom_hs.validators import validate_document
        ...
        records = [
            (record.status, record.evidence)
            for record in doc.checks
            if record.property == "section-has-valid-record-type"
        ]
        self.assertEqual(
            records,
            [
                (CheckStatus.FAIL, "unsupported section record type=NoneType"),
                (CheckStatus.FAIL, "unsupported section record type=list"),
                (CheckStatus.PASS, "record type=Section"),
            ],
        )
    ...
    def test_validation_obligation_rejects_empty_object_subtarget(self):
        doc = SpecDocument(
            objects=[
                SpecObject(
                    "object:child",
        ...
        matches = [
            ...
            for check in doc.checks
            if check.property == "obligation-target-is-declared"
            and check.target_id == "vobl-empty-child"
        ]
        self.assertEqual(len(matches), 1)
        self.assertEqual(
            (matches[0].status, matches[0].evidence),
    ...
    def test_validation_obligation_rejects_padded_object_subtarget(self):
        doc = SpecDocument(
            objects=[
                SpecObject(
                    "object:child",
            ...
            validation_obligations=[
                ValidationObligation(
                    ...
                    "synthetic-property",
                    "object:child: fact:0 ",
                    "Outer whitespace makes the subtarget identity ambiguous.",
                )
            ],
        )
        from specatom_hs.validators import validate_document
        ...
        matches = [
            ...
            for check in doc.checks
            if check.property == "obligation-target-is-declared"
            and check.target_id == "vobl-padded-child"
        ]
        self.assertEqual(len(matches), 1)
        self.assertEqual(matches[0].status, CheckStatus.FAIL)
        self.assertEqual(
    ...
    def test_validation_obligation_rejects_leading_padded_object_subtarget(self):
        doc = SpecDocument(
            objects=[
                SpecObject(
                    "object:child",
            ...
            validation_obligations=[
                ValidationObligation(
                    "vobl-leading-padded-child",
                    "synthetic-property",
                    " object:child:fact:0",
                    "Leading whitespace changes the target identity.",
                )
            ],
        )
        from specatom_hs.validators import validate_document
        ...
        matches = [
            ...
            for check in doc.checks
            if check.property == "obligation-target-is-declared"
            and check.target_id == "vobl-leading-padded-child"
        ]
        self.assertEqual(len(matches), 1)
        self.assertEqual(matches[0].status, CheckStatus.FAIL)
        self.assertEqual(
    ...
    def test_validation_obligation_rejects_space_before_object_subtarget_separator(self):
        doc = SpecDocument(
            objects=[
                SpecObject(
                    "object:child",
            ...
            validation_obligations=[
                ValidationObligation(
                    "vobl-pre-separator-space",
                    "synthetic-property",
                    "object:child :fact:0",
                    "Whitespace before the separator changes the target identity.",
                )
            ],
        )
        from specatom_hs.validators import validate_document
        ...
        matches = [
            ...
            for check in doc.checks
            if check.property == "obligation-target-is-declared"
            and check.target_id == "vobl-pre-separator-space"
        ]
        self.assertEqual(len(matches), 1)
        self.assertEqual(matches[0].status, CheckStatus.FAIL)
        self.assertEqual(
    ...
    def test_validation_obligation_rejects_unicode_space_before_object_subtarget_separator(self):
        doc = SpecDocument(
            objects=[
                SpecObject(
                    "object:child",
            ...
            validation_obligations=[
                ValidationObligation(
                    "vobl-pre-separator-nbsp",
                    "synthetic-property",
                    "object:child\u00a0:fact:0",
                    "Unicode whitespace changes the target identity.",
                )
            ],
        )
        from specatom_hs.validators import validate_document
        ...
        matches = [
            ...
            for check in doc.checks
            if check.property == "obligation-target-is-declared"
            and check.target_id == "vobl-pre-separator-nbsp"
        ]
        self.assertEqual(len(matches), 1)
        self.assertEqual(matches[0].status, CheckStatus.FAIL)
        self.assertEqual(
    ...
    def test_validation_obligation_rejects_unicode_space_after_object_subtarget_separator(self):
        doc = SpecDocument(
            objects=[
                SpecObject(
                    "object:child",
            ...
            validation_obligations=[
                ValidationObligation(
                    "vobl-post-separator-em-space",
                    "synthetic-property",
                    "object:child:\u2003fact:0",
                    "Unicode whitespace changes the subtarget identity.",
                )
            ],
        )
        from specatom_hs.validators import validate_document
        ...
        matches = [
            ...
            for check in doc.checks
            if check.property == "obligation-target-is-declared"
            and check.target_id == "vobl-post-separator-em-space"
        ]
        self.assertEqual(len(matches), 1)
        self.assertEqual(matches[0].status, CheckStatus.FAIL)
        self.assertEqual(
    ...
    def test_validation_obligation_rejects_zero_width_space_after_object_subtarget_separator(self):
        doc = SpecDocument(
            objects=[
                SpecObject(
                    "object:child",
            ...
            validation_obligations=[
                ValidationObligation(
                    "vobl-post-separator-zero-width-space",
                    "synthetic-property",
                    "object:child:\u200bfact:0",
                    "Invisible format padding changes the subtarget identity.",
                )
            ],
        )
        from specatom_hs.validators import validate_document
        ...
        matches = [
            ...
            for check in doc.checks
            if check.property == "obligation-target-is-declared"
            and check.target_id == "vobl-post-separator-zero-width-space"
        ]
        self.assertEqual(len(matches), 1)
        self.assertEqual(matches[0].status, CheckStatus.FAIL)
        self.assertEqual(
    ...
    def test_validation_obligation_rejects_trailing_zero_width_space_in_object_subtarget(self):
        doc = SpecDocument(
            objects=[
                SpecObject(
                    "object:child",
            ...
            validation_obligations=[
                ValidationObligation(
                    "vobl-trailing-zero-width-space",
                    "synthetic-property",
                    "object:child:fact:0\u200b",
                    "Invisible trailing padding changes the subtarget identity.",
                )
            ],
        )
        from specatom_hs.validators import validate_document
        ...
        matches = [
            ...
            for check in doc.checks
            if check.property == "obligation-target-is-declared"
            and check.target_id == "vobl-trailing-zero-width-space"
        ]
        self.assertEqual(len(matches), 1)
        self.assertEqual(matches[0].status, CheckStatus.FAIL)
        self.assertEqual(
    ...
    def test_validation_obligation_rejects_interior_zero_width_space_in_object_subtarget(self):
        doc = SpecDocument(
            objects=[
                SpecObject(
                    "object:child",
            ...
            validation_obligations=[
                ValidationObligation(
                    "vobl-interior-zero-width-space",
                    "synthetic-property",
                    "object:child:fact:\u200b0",
                    "Invisible interior padding changes the subtarget identity.",
                )
            ],
        )
        from specatom_hs.validators import validate_document
        ...
        matches = [
            ...
            for check in doc.checks
            if check.property == "obligation-target-is-declared"
            and check.target_id == "vobl-interior-zero-width-space"
        ]
        self.assertEqual(len(matches), 1)
        self.assertEqual(matches[0].status, CheckStatus.FAIL)
        self.assertEqual(
    ...
    def test_validation_obligation_rejects_interior_control_in_object_subtarget(self):
        doc = SpecDocument(
            objects=[
                SpecObject(
                    "object:child",
            ...
            validation_obligations=[
                ValidationObligation(
                    "vobl-interior-control",
                    "synthetic-property",
                    "object:child:fact:\u00070",
                    "An invisible control character changes the subtarget identity.",
                )
            ],
        )
        from specatom_hs.validators import validate_document
        ...
        matches = [
            ...
            for check in doc.checks
            if check.property == "obligation-target-is-declared"
            and check.target_id == "vobl-interior-control"
        ]
        self.assertEqual(len(matches), 1)
        self.assertEqual(matches[0].status, CheckStatus.FAIL)
        self.assertEqual(
    ...
    def test_validation_obligation_rejects_interior_surrogate_in_object_subtarget(self):
        doc = SpecDocument(
            objects=[
                SpecObject(
                    "object:child",
            ...
            validation_obligations=[
                ValidationObligation(
                    "vobl-interior-surrogate",
                    "synthetic-property",
                    "object:child:fact:\ud8000",
                    "A non-scalar surrogate changes the subtarget identity.",
                )
            ],
        )
        from specatom_hs.validators import validate_document
        ...
        matches = [
            ...
            for check in doc.checks
            if check.property == "obligation-target-is-declared"
            and check.target_id == "vobl-interior-surrogate"
        ]
        self.assertEqual(len(matches), 1)
        self.assertEqual(matches[0].status, CheckStatus.FAIL)
        self.assertEqual(
    ...
    def test_validation_obligation_rejects_interior_noncharacter_in_object_subtarget(self):
        doc = SpecDocument(
            objects=[
                SpecObject(
                    "object:child",
            ...
            validation_obligations=[
                ValidationObligation(
                    "vobl-interior-noncharacter",
                    "synthetic-property",
                    "object:child:fact:\ufdd00",
                    "A reserved Unicode noncharacter changes the subtarget identity.",
                )
            ],
        )
        from specatom_hs.validators import validate_document
        ...
        matches = [
            ...
            for check in doc.checks
            if check.property == "obligation-target-is-declared"
            and check.target_id == "vobl-interior-noncharacter"
        ]
        self.assertEqual(len(matches), 1)
        self.assertEqual(matches[0].status, CheckStatus.FAIL)
        self.assertEqual(
    ...
    def test_validation_obligation_rejects_private_use_character_in_object_subtarget(self):
        doc = SpecDocument(
            objects=[
                SpecObject(
                    "object:child",
            ...
            validation_obligations=[
                ValidationObligation(
                    ...
                    "synthetic-property",
                    "object:child:fact:\ue0000",
                    "Private-use Unicode code points are not portable identities.",
                )
            ],
        )
        from specatom_hs.validators import validate_document
        ...
        matches = [
            ...
            for check in doc.checks
            if check.property == "obligation-target-is-declared"
            and check.target_id == "vobl-private-use"
        ]
        self.assertEqual(len(matches), 1)
        self.assertEqual(matches[0].status, CheckStatus.FAIL)
        self.assertEqual(
    ...
    def test_validation_obligation_rejects_unassigned_character_in_object_subtarget(self):
        doc = SpecDocument(
            objects=[
                SpecObject(
                    "object:child",
            ...
            validation_obligations=[
                ValidationObligation(
                    ...
                    "synthetic-property",
                    "object:child:fact:\u03780",
                    "Unassigned Unicode code points are not stable identities.",
                )
            ],
        )
        from specatom_hs.validators import validate_document
        ...
        matches = [
            ...
            for check in doc.checks
            if check.property == "obligation-target-is-declared"
            and check.target_id == "vobl-unassigned"
        ]
        self.assertEqual(len(matches), 1)
        self.assertEqual(matches[0].status, CheckStatus.FAIL)
        self.assertEqual(
    ...
    def test_validation_obligation_rejects_non_nfc_object_subtarget(self):
        doc = SpecDocument(
            objects=[
                SpecObject(
                    "object:child",
            ...
            validation_obligations=[
                ValidationObligation(
                    ...
                    "synthetic-property",
                    "object:child:cafe\u0301",
                    "Object subtargets must have one canonical Unicode identity.",
                )
            ],
        )
        from specatom_hs.validators import validate_document
        ...
        matches = [
            ...
            for check in doc.checks
            if check.property == "obligation-target-is-declared"
            and check.target_id == "vobl-non-nfc"
        ]
        self.assertEqual(len(matches), 1)
        self.assertEqual(matches[0].status, CheckStatus.FAIL)
        self.assertEqual(
            matches[0].evidence,
            "undeclared target=object:child:cafe\u0301",
        )

    def test_validation_obligation_rejects_non_nfkc_object_subtarget(self):
        doc = SpecDocument(
            objects=[
                SpecObject(
                    "object:child",
        ...
        matches = [
            ...
            for check in doc.checks
            if check.property == "obligation-target-is-declared"
            and check.target_id == "vobl-non-nfkc"
        ]
        self.assertEqual(len(matches), 1)
        self.assertEqual(matches[0].status, CheckStatus.FAIL)
        self.assertEqual(
            matches[0].evidence,
            "undeclared target=object:child:\uff46act:0",
        )

    def test_validation_obligation_rejects_variation_selector_in_object_subtarget(self):
        doc = SpecDocument(
            objects=[
                SpecObject(
                    "object:child",
            ...
            validation_obligations=[
                ValidationObligation(
                    "vobl-variation-selector",
                    "synthetic-property",
                    "object:child:fact\ufe0f:0",
                    "Variation selectors must not create visually ambiguous identities.",
                )
            ],
        )
        from specatom_hs.validators import validate_document
        ...
        matches = [
            ...
            for check in doc.checks
            if check.property == "obligation-target-is-declared"
            and check.target_id == "vobl-variation-selector"
        ]
        self.assertEqual(len(matches), 1)
        self.assertEqual(matches[0].status, CheckStatus.FAIL)
        self.assertEqual(
            matches[0].evidence,
            "undeclared target=object:child:fact\ufe0f:0",
        )

    def test_validation_obligation_rejects_combining_mark_in_object_subtarget(self):
        doc = SpecDocument(
            objects=[
                SpecObject(
                    "object:child",
            ...
            validation_obligations=[
                ValidationObligation(
                    "vobl-combining-mark",
                    "synthetic-property",
                    "object:child:fact\u0338:0",
                    "Combining marks must not create visually ambiguous identities.",
                )
            ],
        )
        from specatom_hs.validators import validate_document
        ...
        matches = [
            ...
            for check in doc.checks
            if check.property == "obligation-target-is-declared"
            and check.target_id == "vobl-combining-mark"
        ]
        self.assertEqual(len(matches), 1)
        self.assertEqual(matches[0].status, CheckStatus.FAIL)
        self.assertEqual(
            matches[0].evidence,
            "undeclared target=object:child:fact\u0338:0",
        )
