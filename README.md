...
- source-file/source-span validation obligations for reproducible file digests, indexed-file byte bounds, byte-offset-derived line numbers, and section/item links to indexed PlainFiles;
...
- validation-layer self-checks that ensure validation obligations cite known source/target provenance, and each check record cites an existing obligation, uses a declared status, preserves non-empty evidence, and matches its declared property/target;
