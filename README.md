...
- first Phase 2 semantic-object slice for explicit `Scope:`/`Context:`, `Epistemic status:`/`Status:`, `Evidence:`, `Interpretation:`, and `Bridge:` markers, producing stable source-provenance-backed objects with exact occurrence source spans (including repeated markers and continuation-line markers) plus conservative Pass/Unknown checks/questions for unsupported status labels, arbitrary unsupported bridge ontology labels, and interpretations without explicit same-item evidence;
