...
- PeTTa profile refusal records for `RawTextOnly`, unsupported semantic levels, unsupported predicates, non-string predicate values, malformed fact record types/arity, object-fact subject mismatches, non-string object IDs, malformed validation-obligation/check IDs, validation-obligation target IDs and rationales, check-to-obligation IDs, and check properties, undeclared check-status values, non-string or whitespace-only object and validation-obligation source-span IDs, and non-string object references;
