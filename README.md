...
- first Phase 2 semantic-object slice for explicit `Scope:`/`Context:`, `Epistemic status:`/`Status:`, `Evidence:`, `Interpretation:`, and `Bridge:` markers, producing stable source-provenance-backed objects plus conservative Pass/Unknown checks/questions for unsupported status labels, unsupported bridge ontologies, and interpretations without explicit same-item evidence;
